...
from config.paths import MALE_PLAYERS_CSV, MALE_PLAYERS_SORTED_CSV, FOOTBALL_TEAMS_SORTED, FOOTBALL_TEAMS
...
def charger_donnees_equipe():
    ...
    """
    try:
        logger.info(f"Chargement des données depuis le fichier '{FOOTBALL_TEAMS}'.")
        return pd.read_csv(FOOTBALL_TEAMS)
    except FileNotFoundError:
        logger.error("Exception -> Le fichier spécifié n'existe pas.")
        return None
    except pd.errors.ParserError:
...
def traiter_donnees_equipes():
    """
    Fonction principale pour le traitement des données.
    """
    # Charger les données
    data = charger_donnees_equipe()

    if data is not None:
        # Vérifier si les données ont déjà été traitées pour ne pas refaire le traitement à chaque fois
        if 'Catégorie' not in data.columns :
            # Catégoriser les joueurs
            data = categoriser_equipe(data)

            # Enregistrer les données traitées dans un nouveau fichier CSV
            nouveau_fichier_csv = FOOTBALL_TEAMS_SORTED
            sauvegarder_donnees(data, nouveau_fichier_csv)
            logger.info(f"Les données ont été traitées et enregistrées dans : {nouveau_fichier_csv}.")
